...
export async function POST(req: Request) {
  try {
    ...
    console.log('[2FA] code recebido:', code)
    ...
    console.log('[2FA] user id:', user?.id)
    ...
    const { data: profile, error } = await supabase
      ...
      .eq('id', user!.id)
      .maybeSingle()
    console.log('[2FA] db error:', error?.message, '| secret:', profile?.totp_secret)
    ...
    console.log('[2FA] esperado agora:', totp.generate(), '| recebido:', code)
